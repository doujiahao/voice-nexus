...
- 服务端统一管理账号、JWT、RBAC、通话会话、坐席状态、音频接收与存储、AI 编排、DB/Redis/MinIO 访问。
- Web 端只保留登录态、页面展示、WebSocket 接收、坐席接听/挂断等操作入口。
- `freeswichService` 负责 FreeSWITCH 控制、ESL 事件监听、路由申请、RTMP 接收与解码、声道分离、VAD 断句、WAV 封装、音频上行、事件上报和控制指令下发。
- `Call_Handing_Gateway` 只提供 ASR、NLP、AgentAssist、TTS 等原子 AI 接口（完全无状态，不维护通话上下文）。
...
graph LR
    ...
    D[freeswichService<br/>ESL 监听<br/>fs_cli/ESL 控制<br/>路由申请<br/>RTMP 解码/声道分离/VAD/WAV<br/>音频上行<br/>事件上报]:::box

    E[JavaService / 统一服务端<br/>账号与权限<br/>呼入路由<br/>通话会话<br/>音频接收与存储<br/>AI 编排<br/>DB/Redis/MinIO 读写<br/>WebSocket 推送]:::core
...
flowchart TB
  ...
  subgraph Media["媒体与控制适配层"]
    ...
    FSS["freeswichService\n无状态 FreeSWITCH 控制面\nESL 监听 / fs_cli 控制 / RTMP 解码\n声道分离 / VAD 断句 / WAV 封装 / 音频上行"]
  end
  ...
  subgraph ServerLayer["统一服务端"]
    ...
    AudioPipeline["音频接收与存储\n接收 WAV 段 / MinIO 存储 / 预签名 URL"]
...
| freeswichService | 无状态 FreeSWITCH 控制面 | ESL 监听、路由申请、fs_cli 控制、RTMP 接收与解码、声道分离、VAD 断句、WAV 封装、音频帧上行、结束事件上报 | 数据库存储、Redis 状态中心、ASR、NLP、坐席路由决策 |
| 统一服务端 | 唯一入口与状态中心 | JWT/RBAC、呼入路由、通话会话、音频接收与存储、AI 编排（维护全量通话上下文）、DB/Redis/MinIO、WS 推送 | SIP/RTP 媒体交换、音频解码/分离/VAD、模型推理实现 |
...
| 16-17 | `WS /internal/ws/audio` | freeswichService 将经过解码、声道分离、VAD 断句后的 WAV 语音段上行给服务端 | JavaService / 统一服务端 | freeswichService | Protobuf AudioFrame: `data`(WAV 字节), `speaker_id`, `speaker_name`, `speaker_role`(customer/agent), `language`, `is_last`, `sequence_number`；Query: `fs_call_id`；Header: `X-Internal-Key` | WS ACK JSON: `sequence`, `accepted`, `call_session_id`, `turn_id` |
...
- 允许保留进程内 ESL 连接、当前 RTMP 接收连接、FFmpeg 子进程、VAD 状态（语音缓冲区/静音帧计数）、短生命周期命令执行状态。
...
- 不允许保存通话历史，不维护通话上下文（上下文由服务端传入），不做用户隔离，不直接访问 DB/Redis/MinIO。
- 所有调用必须来自服务端并携带内部认证头。
